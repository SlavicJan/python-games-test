...
class IsoMap:
    ...
    def draw(self, surf: pygame.Surface, cam_x: int, cam_y: int, hero=None):
        """Draw map + optional hero marker/path.

        In earlier iterations we called: `iso.draw(screen, cam_x, cam_y, hero)`.
        Later the method signature was simplified, which makes that old call
        crash with: `takes 4 positional arguments but 5 were given`.

        This version keeps the old call working and also draws:
        - planned path dots (hero.path)
        - a simple hero "pin" marker
        """
        ...
        # Path dots
        for (gx, gy) in getattr(hero, "path", []) or []:
            ix, iy = grid_to_iso(gx, gy)
            sx = int(ix + cam_x + W // 2)
            sy = int(iy + cam_y + H // 4 + TILE_H // 2)
            pygame.draw.circle(surf, (255, 215, 0), (sx, sy), 4)

        # Hero marker (a small pin)
        ix, iy = grid_to_iso(getattr(hero, "gx", 0), getattr(hero, "gy", 0))
        sx = int(ix + cam_x + W // 2)
        cy = int(iy + cam_y + H // 4 + TILE_H // 2)
        head_y = cy - 16
        pygame.draw.line(surf, (245, 245, 245), (sx, head_y + 7), (sx, cy - 2), 3)
        pygame.draw.circle(surf, (245, 245, 245), (sx, head_y), 7)
...
def main():
    ...
    # Camera origin (screen pixel coords). Aim at safe_rect center.
    cam_x = safe_rect.centerx
    cam_y = safe_rect.top + 140
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_ESCAPE:
                    ...
                    cam_x = safe_rect.centerx - int(hsx)
                    cam_y = safe_rect.centery - int(hsy)
                    msg = "Камера по герою."
