...
class IsoMap:
    def __init__(self):
        ...
        # Defaults (may be overwritten in main).
        # Keeping them here prevents AttributeError in draw().
        self.portal_gx = GRID_W // 2
        self.portal_gy = GRID_H // 2
        # Enemies are managed in main (a list[Enemy]). We keep a safe default
        # so draw() can iterate without crashing.
        self.enemies = []

        # Ensure the default portal tile is walkable.
        self.blocked.discard((self.portal_gx, self.portal_gy))
